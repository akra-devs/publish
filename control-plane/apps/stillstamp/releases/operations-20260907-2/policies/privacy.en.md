...
stillstamp is operated by AKRA (akra-devs). Privacy questions may be sent to
...
For the real mobile service, stillstamp uses a screenless anonymous Firebase
identity. There is no email, password, social login, or signup screen. Firebase
Authentication and App Check process the anonymous identifier, authentication
metadata, IP address or user-agent security signals, and app/device attestation
material to authenticate requests and reduce abuse.
...
rewarded-ad receipts, and related anti-replay records have no automatic expiry
while the account remains active and are deleted with the cloud account.
...
Ads and UMP, Amazon Web Services in the Seoul region, OpenAI's API, and GitHub's
raw-content service for AKRA configuration delivery. Signature enforcement for
that configuration is not enabled yet. Google Play
...
wallet, attendance and generation records, rewarded-ad receipts, and temporary
generated results, and then deletes the anonymous Firebase identity. The
...
stillstamp is not designed specifically for children. The final store target-age
declaration remains a release review item. Users must have the right to upload

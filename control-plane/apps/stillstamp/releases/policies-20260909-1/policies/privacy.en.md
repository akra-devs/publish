...
the account remains active and are deleted with the cloud account. Purchase
transaction and intent records remain without the raw Firebase UID and without
automatic expiry to prevent purchase replay.
...
### First-use credit and device records

On Android, the app sends the Android ID over HTTPS to the AKRA backend when
requesting first-use credit. The server uses a dedicated secret to derive an
HMAC reference and does not persist or log the raw Android ID. It retains only
the derived reference, first-use policy and recording time to prevent repeated
starter grants after reinstalling the app or deleting a cloud account. This
record does not contain your Firebase UID and is not used to restore accounts,
track activity across other apps, or deliver advertising.

The device record remains while this first-use offer operates, including after
cloud account deletion; it has no automatic expiry. It is removed when the
first-use offer and its abuse-prevention need are retired. Questions about this
record or a change of device ownership can be sent to help@akra.kr. This limited
record is separate from the account records deleted by the cloud-account flow.
If the device signal is unavailable, first-use credit is withheld; existing
credits and independently verified purchase or advertising rewards remain
available.
...
On Android, open **My info → Account · recover credits → Delete cloud account**. After confirmation, the app locks cloud features, deletes the AWS
...
other unnecessary personal information. Visit https://stillstamp.akra.kr/delete-account
for retention details and how to request deletion when you cannot use the app.

## 7. Children, user responsibility, and rights

stillstamp is intended for users aged 13 and over and is not a child-directed
service. Users must have the right to upload
...
or at https://stillstamp.akra.kr/privacy.
